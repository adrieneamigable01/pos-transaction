$(()=>{
    ...
    pos = {
        ...
        ajax:{
            ...
            getproductType:()=>{
                return new Promise((resolve,reject)=>{
                    ...
                    .on('select2:unselect', function (e) {
                        var selectedValues = $(this).val();  // Step 1: Get currently selected values 
                        productTypeId = selectedValues;
                        pos.ajax.getProducts();
                    })
                    .on('select2:select', function (e) {
                        var selectedData = e.params.data; // The selected data object
                        var selectedValues = $(this).val();  // Step 1: Get currently selected values 
                        console.log('Item added:', JSON.stringify(selectedData));
                    
                        if(selectedData.id != "all"){
            
                            if(selectedValues.indexOf("all") >= 0){
                             
                                var valueToRemove = 'all';  // Example: Value you want to remove
            
                                // Step 2: Remove the specific value from the selected values array
                                var indexToRemove = selectedValues.indexOf(valueToRemove);
                                if (indexToRemove !== -1) {
                                    selectedValues.splice(indexToRemove, 1);
                                }
            
                                // Step 3: Update Select2 with the modified array of values and trigger change event
                    ...
                    $('#product_type').val("all").trigger('change');
                    pos.ajax.getProducts();
                })
            },
            getStockConversion:()=>{
            ...
            getProductsWConversion:(payload,productid)=>{
                return new Promise((resolve,reject)=>{
                    jsAddon.display.ajaxRequest({
                        type:'get',
                        url:getProductsApi,
        ...
        display:{
            ...
            displayDimensionConvesion:(selectedValue)=>{
                let totalBoardFeet = selectedValue.quantity * selectedValue.size;
                let length = 0;
                let width = 0;
                let thickness = 0;
                ...
                pos.ajax.getProductsWConversion({productTypeId:selectedValue.productTypeId},selectedValue.productid);
                    ...
                    $.each(materialsData,function(key,value){
                        ...
                        $("#material-container tbody").append(
                            ...
                            .append(
                                ...
                                $("<td>").text(value.selling_price),
                                $("<td>").text(`${value.unit} (${value.abbreviations})`),
                                $("<td>").text(`${value.size == null ? value.total  : parseFloat(value.total).toFixed(2)}`),
                            )
                        )
            ...
            displayProductList:() => {
                ...
                $.each(stockData,function(key,value){
                    ...
                    const newItem = $("<div class='col-md-4 mb-4'>").append(
                        $("<div class='card'>").append(
                            ...
                            .attr({
                                src: value.image != "" ? `${baseApiUrl}/uploads/product/${value.productid}/${value.image}` : 'https://th.bing.com/th/id/OIP.n0lQ1Wu8fn90-UEDS6-FBAHaGv?w=197&h=180&c=7&r=0&o=5&dpr=1.3&pid=1.7',
                                alt: value.product_name,
                                class: "card-img-top",
                                height:300
                            }),
                            $("<div class='card-body'>").append(
                                $("<h5 class='card-title'>").text(`${value.productType}`),
                                $("<span>").text(`(${value.product_name})`),
                                $("<p class='card-text'>").text(jsAddon.display.formatNumber(parseFloat(value.selling_price))),
                                $("<p class='card-text' style='position: absolute; bottom: 10px; right: 10px;'>") // Add this line for positioning
                                .text(
                                    `Stock ${value.size == null ? value.total - usedStocks : (value.total - usedStocks).toFixed(2)}`
                                ) // Assuming `remaining_stock` is part of your `value` object
                                .css({
                                    "font-size": "0.9em", // Adjust font size as needed
                                    "color": "#6c757d" // Optional: change color for better visibility
                                })
                            )
                        )
                    ).click(function(){
                    ...
                    $('#stock-container').append(newItem);
                    ...
                    //         $('#product-container tr.table-secondary').removeClass("table-secondary");
                    //         $('#product-container tr').eq(currentRowIndex).addClass('table-secondary');
                    //     })
                    //     .dblclick(function(){
                    //         selectedStack = JSON.stringify(value);
                    //         pos.display.addToCart()
    ...
    $("#product-search").on('keyup',function(){
        ...
        $('#product-container tr').each(function() {
                ...
                .append(
                    $("<tr>")
                        .append(
                            ...
                            $("<td>")
                                .text(`${value.quantity}(${value.abbreviations})`),
                            $("<td>")
                                .append(
                                    $("<span>")
                                    .text(value.price),
